...
impl TavernCardV2 {
    ...
    pub fn from_png_image(image_data: &Bytes) -> Result<Self> {
        ...
        info!("Attempting to deserialize as TavernCardV2...");
        let mut card = serde_json::from_slice::<TavernCardV2>(&text);
        if card.is_err() {
            debug!("Failed to deserialize as TavernCardV2: {:?}", card.as_ref().err());
            info!("Attempting to deserialize as CharacterData (data portion only)...");
            // Sometimes the tag contains only the data portion
            match serde_json::from_slice::<CharacterData>(&text) {
                Ok(card_data) => {
                    info!("Successfully deserialized as CharacterData.");
            ...
            info!("Successfully deserialized as TavernCardV2.");
        }
